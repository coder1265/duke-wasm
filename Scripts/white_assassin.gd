...
var min_left = get_node("/root/Main").min_left
var min_right = get_node("/root/Main").min_right
var min_top = get_node("/root/Main").min_top
var min_bottom = get_node("/root/Main").min_bottom
...
func do_moves():
	...
	if is_front:
		jump_array = [
			Vector2i(0,1),Vector2i(0,2),Vector2i(0,3),Vector2i(0,4), Vector2i(0,5), Vector2i(0,6),
...
func next_turn():
	if is_front:
		is_front = false
		$assassin_img.play("back")
	else:
		is_front = true
		$assassin_img.play("front")
